...
public static class SunshineKeywords
{
	...
	private class ChangeTracker
	{
		...
		public bool Change(bool newValue) { return Change(newValue ? 1 : 0); }
	...
	private static readonly string[] X_CASCADES = new string[] { ONE_CASCADE, TWO_CASCADES, THREE_CASCADES, FOUR_CASCADES };
	private static ChangeTracker cascadeCount = new ChangeTracker();
	public static void SetCascadeCount(int i) { if(cascadeCount.Change(Mathf.Clamp(i-1, 0, 3))) SetKeyword(cascadeCount.Value, X_CASCADES); }
	...
	public static void ToggleOvercast(bool enabled) { if(overcast.Change(enabled)) ToggleKeyword(enabled, OVERCAST_ON, OVERCAST_OFF); }
	...
	private static readonly string[] FILTER_STYLES = new string[] { FILTER_DISABLED, FILTER_HARD, FILTER_PCF_2x2, FILTER_PCF_3x3, FILTER_PCF_4x4};
	public static void SetFilterStyle(int style) { SetKeywordWithFallbacks(style, FILTER_STYLES, 1); }
	public static void SetFilterStyle(SunshineShadowFilters style) { SetFilterStyle((int)style+1); }
	public static void DisableShadows() { SetFilterStyle(0); }
	
	// To save Shader Keywords, we'll recycle Filter Keywords:
	private const string SCATTER_QUALITY_LOW = FILTER_HARD;//"SUNSHINE_SCATTER_QUALITY_LOW";
	private const string SCATTER_QUALITY_MEDIUM = FILTER_PCF_2x2;//"SUNSHINE_SCATTER_QUALITY_MEDIUM";
	private const string SCATTER_QUALITY_HIGH = FILTER_PCF_3x3;//"SUNSHINE_SCATTER_QUALITY_HIGH";
	private const string SCATTER_QUALITY_VERYHIGH = FILTER_PCF_4x4;//"SUNSHINE_SCATTER_QUALITY_VERYHIGH";
	private static readonly string[] SCATTER_QUALITIES = new string[] { SCATTER_QUALITY_LOW, SCATTER_QUALITY_MEDIUM, SCATTER_QUALITY_HIGH, SCATTER_QUALITY_VERYHIGH };
	public static void SetScatterQuality(SunshineScatterSamplingQualities quality) { SetKeyword((int)quality, SCATTER_QUALITIES); }
