...
public class PlayerAnimationMecanim : MonoBehaviour
{
    ...
    public DirectionAnimationMecanim[] moveAnimations;
    ...
    private DirectionAnimationMecanim bestAnimation = null;
    private float lastFootstepTime = 0;
    private float lastAnimTime = 0;
    public float constantSmooth = 0;
    private static PlayerAnimationMecanim _entity;
    public static PlayerAnimationMecanim entity
    {
        get { return _entity; }
        set { _entity = value; }
    }
    ...
    public void Awake()
    {
        ...
        foreach (DirectionAnimationMecanim moveAnimation in moveAnimations)
        {
            moveAnimation.Init();
    ...
    public void Update()
    {
        if (!Player.entity.isDeath)
        {
            ...
            if (speed > 1)
            {
                ...
                //Debug.Log(bestAnimation.AnimName + "  " + speed);
                foreach (DirectionAnimationMecanim anim in moveAnimations)
                {
                    if (anim.AnimName != bestAnimation.AnimName)
                    {
                        _anim.SetBool(anim.AnimName, false);
                    }
                    else
                    {
                        _anim.SetBool(anim.AnimName, true);
                        ChangeWeight(anim.AnimName);
                    }
                }
            ...
            else
            {
                ...
                foreach (DirectionAnimationMecanim a in moveAnimations)
                {
                    // if (a.AnimName != idle) _anim.SetBool(a.AnimName, false);
                    _anim.SetBool(a.AnimName, false);
                }
    ...
    void ChangeWeight(string AnimName)
    {
        foreach (DirectionAnimationMecanim anim in moveAnimations)
        {
            anim.weight = anim.AnimName == AnimName ? 1f : 0;
        }
    }
            ...
            if (idle < 1)
            {
                ...
                foreach (DirectionAnimationMecanim moveAnimation in moveAnimations)
                {
                    ...
                    if (Vector3.Dot(moveAnimation.velocity, localVelocity) <= 0)
                       ...
                       animatedLocalVelocity += moveAnimation.velocity * moveAnimation.weight;
                   // animatedLocalVelocity += moveAnimation.velocity * 1;
                }
...
[System.Serializable]
public class DirectionAnimationMecanim
{
    //клип анимации
    ...
    public void Init()
    {
        // _animClip.SetBool(AnimName, true);
